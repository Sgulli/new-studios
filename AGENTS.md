...
- **Prefix Node.js built-in modules with `node:`** when importing (e.g. `import path from "node:path"`, `import fs from "node:fs"`). Do not use unprefixed specifiers like `"path"` or `"fs"`.
